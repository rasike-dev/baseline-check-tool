...
exports.deactivate = exports.activate = exports.BaselineCheckProvider = void 0;
...
class BaselineCheckProvider {
    ...
    async runBaselineCheckCommand(command, args = []) {
        try {
            ...
            if (!baselineCheckPath) {
                throw new Error('Baseline Check Tool not found. Please install it:\n' +
    ...
    convertReportToExpectedFormat(report) {
        ...
        if (results.length > 0) {
            baselineLike = results.filter((r) => r.status === 'baseline_like').length;
        ...
        return {
            ...
            results: results.length > 0 ? results : detected.map((feature) => ({
    ...
    parseScanOutput(stdout) {
        ...
        for (const line of lines) {
            ...
            if (line.includes('baseline-like')) {
                ...
                if (match)
                    baselineLike = parseInt(match[1]);
            }
            if (line.includes('risky')) {
                const match = line.match(/(\d+)/);
                if (match)
                    risky = parseInt(match[1]);
            }
            if (line.includes('unknown')) {
                const match = line.match(/(\d+)/);
                if (match)
                    unknown = parseInt(match[1]);
            }
        }
        ...
        if (baselineLike === 0 && risky === 0 && unknown === 0) {
            for (const line of lines) {
                if (line.includes('✅') && line.includes('⚠️') && line.includes('❓')) {
                    ...
                    if (baselineMatch)
                        baselineLike = parseInt(baselineMatch[1]);
                    if (riskyMatch)
                        risky = parseInt(riskyMatch[1]);
                    if (unknownMatch)
                        unknown = parseInt(unknownMatch[1]);
                }
            }
        }
        return {
    ...
    createDiagnostics(results, filePath) {
        const diagnostics = [];
        results.forEach(result => {
            if (result.files.some(f => f === path.basename(filePath))) {
                ...
                const diagnostic = new vscode.Diagnostic(new vscode.Range(0, 0, 0, 0), // Will be updated with actual position
                message, severity);
                diagnostic.source = 'Baseline Check';
                diagnostic.code = result.feature;
                if (result.mdn) {
                    diagnostic.relatedInformation = [
                        new vscode.DiagnosticRelatedInformation(new vscode.Location(vscode.Uri.parse(result.mdn), new vscode.Range(0, 0, 0, 0)), 'MDN Documentation')
                    ];
                }
    ...
    async scanFile(filePath) {
        try {
            ...
            if (vscode.workspace.getConfiguration('baseline-check').get('showNotifications')) {
                const summary = results.summary || {};
                vscode.window.showInformationMessage(`Baseline scan completed: ${summary.baselineLike || 0} baseline, ${summary.risky || 0} risky, ${summary.unknown || 0} unknown`);
            }
        }
        catch (error) {
    ...
    async scanWorkspace() {
        ...
        try {
            ...
            if (vscode.workspace.getConfiguration('baseline-check').get('showNotifications')) {
                const summary = results.summary || {};
                const result = await vscode.window.showInformationMessage(`✅ Scan completed: ${summary.baselineLike || 0} baseline, ${summary.risky || 0} risky, ${summary.unknown || 0} unknown`, 'View Dashboard', 'Show Output');
    ...
    async runFullAnalysis() {
        ...
        try {
            ...
            const result = await vscode.window.showInformationMessage('✅ Full analysis completed! Dashboards updated.', 'Open Dashboard', 'Show Output');
            if (result === 'Open Dashboard') {
                // Always open a fresh dashboard
                const dashboardPath = path.join(workspaceFolder.uri.fsPath, 'dashboards', 'index.html');
                const uri = vscode.Uri.file(dashboardPath);
                await vscode.env.openExternal(uri);
                vscode.window.showInformationMessage('📊 Dashboard opened. If you see old data, press Cmd+R to refresh the browser.', 'OK');
            }
            else if (result === 'Show Output') {
                this.outputChannel.show();
            }
    ...
    async runPerformanceAnalysis() {
        try {
            ...
            // Show notification with button to open dashboard
            const result = await vscode.window.showInformationMessage('⚡ Performance analysis completed!', 'View Dashboard', 'Show Output');
            if (result === 'View Dashboard') {
                this.openDashboard();
            }
            else if (result === 'Show Output') {
    ...
    async runSecurityAnalysis() {
        try {
            ...
            vscode.window.showInformationMessage('Security analysis completed! Check the output panel for details.');
            this.outputChannel.show();
        }
        catch (error) {
    ...
    async runAccessibilityAnalysis() {
        try {
            ...
            vscode.window.showInformationMessage('Accessibility analysis completed! Check the output panel for details.');
            this.outputChannel.show();
        }
        catch (error) {
    ...
    async runSEOAnalysis() {
        try {
            ...
            vscode.window.showInformationMessage('SEO analysis completed! Check the output panel for details.');
            this.outputChannel.show();
        }
        catch (error) {
    ...
    async runBundleAnalysis() {
        try {
            ...
            vscode.window.showInformationMessage('Bundle analysis completed! Check the output panel for details.');
            this.outputChannel.show();
        }
        catch (error) {
    ...
    async openDashboard() {
        ...
        if (fs.existsSync(dashboardPath)) {
            ...
            try {
                ...
                vscode.window.showWarningMessage('Dashboard generation failed. Please run an analysis first.', 'Run Analysis').then(selection => {
    ...
    async generateSimpleDashboard(workspacePath) {
        try {
    ...
    generateDashboardHub(workspacePath) {
        return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body>
    ...
    async startMonitoring() {
        ...
        try {
            ...
            const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
            if (workspaceFolder) {
                this.monitoringWatcher = vscode.workspace.createFileSystemWatcher(new vscode.RelativePattern(workspaceFolder, '**/*.{js,ts,jsx,tsx,html,css}'));
    ...
    async fixIssues() {
        try {
            ...
            const results = await this.runBaselineCheckCommand('analyze', ['-r', 'baseline-report.json']);
            vscode.window.showInformationMessage(`Analysis completed: ${results.suggestions?.length || 0} recommendations found`);
            this.statusBarItem.text = "$(check) Baseline Check";
        }
        catch (error) {
    ...
    async generateReport() {
        try {
            ...
            const results = await this.runBaselineCheckCommand('run', ['.']);
            vscode.window.showInformationMessage('Report generated successfully! Check the workspace for baseline-report.json');
            this.statusBarItem.text = "$(check) Baseline Check";
        }
        catch (error) {
...
exports.BaselineCheckProvider = BaselineCheckProvider;
let provider;
function activate(context) {
    ...
    vscode.window.showInformationMessage('Baseline Check Tool is now active! Use the command palette to run analyses.', 'Run Scan').then(selection => {
